...
#ifdef GCWZERO // include needed libraries
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
#endif
...
#ifdef GCWZERO // change filemanager titlebar
static char *text_str_load_title="Select A:FD-A or Y:FD-B";
#else
...
#ifdef GCWZERO // get current diskimage directory
	char cwd[1024];
	if (getcwd(cwd, sizeof(cwd)) != NULL) {
		FILE * fp;
		fp=fopen("/home/retrofw/.DCaSTaway/menudirdefault", "w");
		fprintf(fp, "%s", cwd);
		fclose(fp);
	}
#endif
...
#ifdef GCWZERO //find if $HOME/.DCaSTaway/menudirdefault file present
	char* menudirdef="/";
	FILE * fp;

	fp=fopen("/home/retrofw/.DCaSTaway/menudirdefault", "r");

	if (fp==NULL) {
		return(getFiles(MENU_DIR_DEFAULT));
	} else {
		char userdefaultdir[1000];
		while (fgets(userdefaultdir, 1000, fp) != NULL) {
			printf("%s",userdefaultdir);
		}

		//remove newline
		char *pos;
		if ((pos=strchr(userdefaultdir, '\n')) != NULL) {
			*pos = '\0';
		}

		fclose(fp);

//The following code block borrowed from stackoverflow.com and modified as I was stuck for inspiration at the time.
		/*but has SD card been removed??? */
		struct stat s;
		int err = stat(userdefaultdir, &s);
		if(-1 == err) {
		    if(ENOENT == errno) {        	       /* yes it has! */
				return(getFiles(MENU_DIR_DEFAULT)); //backup!
		    }
		} else {
		    if(S_ISDIR(s.st_mode)) {
			return(getFiles(userdefaultdir));      /* nope, still there */
		    } else {				       /* somethings gone wrong, it's there but it's not a dir! */
		 		return(getFiles(MENU_DIR_DEFAULT)); //backup!
		    }
		}
	}
