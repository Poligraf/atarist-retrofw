...
#ifdef GCWZERO //include required libs
#include<sys/stat.h>
#endif
...
#ifdef GCWZERO //swap labels
static char *text_str_load="Filemanager (Y)";
static char *text_str_save="SaveStates (X)";
#else
...
#ifdef GCWZERO //Hijack vsync label ;)
static char *text_str_vsync="A-Stick";
int gcw_use_analog_pad=0;
#else
...
//	static int b=0;
//	int bb=(b%6)/3;
#ifdef GCWZERO //is gcw_use_analog_pad TRUE?
	FILE * fp;
	fp=fopen("/home/retrofw/.DCaSTaway/.apad", "r");
		if (fp == NULL) { //FALSE
			gcw_use_analog_pad=0;
			mainMenu_vsync=0;

		} else { //TRUE
			gcw_use_analog_pad=1;
			mainMenu_vsync=1;
			fclose(fp);
		}
#endif
		...
		if (event.type == SDL_KEYDOWN)
		{
			...
			switch(c)
			{
				...
				case 4:
					if (left)
					{
#ifdef GCWZERO //set a-pad to joystick
						gcw_use_analog_pad=0;
						mainMenu_vsync=0;
						remove("/home/retrofw/.DCaSTaway/.apad");
					...
					{
#ifdef GCWZERO //set a-pad to mouse
						gcw_use_analog_pad=1;
						mainMenu_vsync=1;
						FILE * fp;
						fp=fopen("/home/retrofw/.DCaSTaway/.apad", "w");
						fprintf(fp, "true");
						fclose(fp);
...
void drawNoRom(void)
{
	int i;
	SDL_Event ev;
#ifdef GCWZERO //create required directories
	mkdir("/home/retrofw/.DCaSTaway",S_IRWXU);
	mkdir("/home/retrofw/.DCaSTaway/bios",S_IRWXU);
	mkdir("/home/retrofw/.DCaSTaway/diskimages",S_IRWXU);
#endif
	for(i=0;i<20;i++)
	{
//		menu_moving=0;
...
#elif GCWZERO //change "ROM not found" message
		write_text(7,16,"Dir created, place in bios/");
#else
