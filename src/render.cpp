...
#ifdef GCWZERO //choose any colour as long as it's black :D
 	render_pal16_copy0=0;
#endif
}
...
void render_up_screen(void)
{
	...
	{
		int i;
#ifdef GCWZERO //bugfix skips every other line otherwise
...
void render_down_screen(void)
{
	...
	{
		int i;
#ifdef GCWZERO //bugfix otherwise skips every other line
		for(i=1;i<=he;i+=1)
#else
		for(i=2;i<=he;i+=2)
#endif
...
void render_status(void)
{
...
#ifdef GCWZERO //doublebuffering requires to draw 2 frames to avoid flicker
	static int redrawscreen=0;
	if (emulated_mouse) {
		vkbd_mouse();
		redrawscreen=1;
	}
	...
	if (show_message)
	{
		show_message--;
		if (show_message) {
#ifdef GCWZERO //y must be greater if we're in medium resolution (screen=640x480)
			if (!actual_is_med)
				_write_text_inv_n(screen,0,28,37,show_message_str);
			else
				_write_text_inv_n(screen,0,57,72,show_message_str);
#else
			_write_text_inv_n(screen,0,28,37,show_message_str);
#endif
		}
	...
	TV_Flip(screen);
#ifdef GCWZERO //remove flicker
	if (vkbd_mode){
		render_force_background();
		render_up_screen();
		vkbd_redraw();
	} else if (emulated_mouse){
		if (redrawscreen == 1) {
			render_force_background();
			render_down_screen();
			vkbd_mouse();
